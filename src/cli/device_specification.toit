...
class DeviceSpecification:
  ...
  constructor.from_json --path/string data/Map:
    ...
    return DeviceSpecification
      ...
      --max_offline_seconds=parse_max_offline_ (data["max-offline"])
  ...
  static parse_max_offline_ max_offline_string/string -> int:
    ...
    last_number_end := 0
    last_unit := null
    max_offline_string.size.repeat: | i |
      c := max_offline_string[i]
      if c == 'h' or c == 'm' or c == 's':
        if state != READING_DIGITS and state != EXPECT_UNIT:
        ...
        if c == 'h':
          ...
          if last_unit != 'h' and last_unit != null:
            throw "Invalid max offline string xxx: $max_offline_string"
          multiplier = 60
        else:
          if last_unit == 's':
            throw "Invalid max offline string  xxxx: $max_offline_string"
          multiplier = 1
        last_unit = c
        seconds += (int.parse max_offline_string[last_number_start..last_number_end]) * multiplier
        last_number_start = i + 1
        state = START_DIGITS
      else if c == ' ':
        if state == READING_DIGITS: state = EXPECT_UNIT
      else if '0' <= c <= '9':
        if state == EXPECT_UNIT:
          throw "Invalid max offline string xxxxx: $max_offline_string"
        else if state == START_DIGITS:
          last_number_start = i
          last_number_end = i + 1
          state = READING_DIGITS
        else if state == READING_DIGITS:
          last_number_end = i + 1
      else:
        throw "Invalid max offline string yy: $max_offline_string"
    if state != START_DIGITS:
      throw "Invalid max offline stringyyy: $max_offline_string"
    return seconds
