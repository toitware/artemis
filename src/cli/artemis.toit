...
class Artemis:
  ...
  */
  customize_envelope
    ...
    with_tmp_directory: | tmp_dir |
      ...
      device_specification.containers.do: | name/string container/Container |
        ...
        apps[name] = build_app_description_ --id=(extract_id_from_snapshot snapshot_path)
  ...
  /**
  Builds an app description as needed for an "apps" entry in the device state.
  */
  build_app_description_ --id/string:
    return {
      "id": id,
    }
  ...
  */
  ...
  app_install --device_id/string --app_name/string --application_path/string:
    update_goal --device_id=device_id: | device/DeviceDetailed |
      ...
      apps[app_name] = build_app_description_ --id=id
      new_goal["apps"] = apps
      new_goal
